...
async def ingest_contract(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Run parsing and extraction in parallel
        await asyncio.gather(
            text_extraction.llama_parse(pdf_path=save_path),
            sla_metadata.run_extraction(pdf_path=save_path)
        )
...
def monitor_sla(request: MonitorRequest):
    ...
    if not news_artifact or not news_artifact.articles:
        return {
            "contract_id": request.contract_id,
            "supplier_name": supplier_name,
